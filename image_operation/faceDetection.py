...
class faceDetection:
    def __init__(self, haarcascades=""):
        self.haarcascadesPath = haarcascades
    ...
    def detectFaces(self, image_name):
        ...
        face_cascade = cv2.CascadeClassifier(self.haarcascadesPath+"/haarcascade_frontalface_default.xml")
        if img.ndim == 3:
    ...
    def detectEyes(self, image_name):
        eye_cascade = cv2.CascadeClassifier(self.haarcascadesPath+'/haarcascade_eye.xml')
        faces = self.detectFaces(image_name)
    ...
    def detectSmiles(self, image_name):
        img = cv2.imread(image_name)
        smiles_cascade = cv2.CascadeClassifier(self.haarcascadesPath+"/haarcascade_smile.xml")
        if img.ndim == 3:
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        else:
            gray = img #if语句：如果img维度为3，说明不是灰度图，先转化为灰度图gray，如果不为3，也就是2，原图就是灰度图
...
if __name__ == '__main__':
    haarcascadesPath = "/usr/local/Cellar/opencv/3.4.1_2/share/OpenCV/haarcascades"
    img = "feifei.jpg"

    pc = faceDetection(haarcascadesPath)
    pc.drawFaces(img)
    pc.drawEyes(img)
    pc.drawSmiles(img)
    pc.saveFaces(img)
