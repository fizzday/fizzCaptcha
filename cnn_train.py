...
from fizzCaptcha.util import next_batch
from fizzCaptcha.captcha_gen import CAPTCHA_HEIGHT, CAPTCHA_WIDTH, CAPTCHA_LEN, CAPTCHA_LIST
...
def train(height=CAPTCHA_HEIGHT, width=CAPTCHA_WIDTH, y_size=len(CAPTCHA_LIST)*CAPTCHA_LEN):
    ...
    while 1:
        ...
        if step % 10 == 0:
            ...
            # 偏差满足要求，保存模型
            if acc >= 0.02:
                model_path = os.getcwd() + os.sep + "model/" + str(acc_rate) + "captcha.model"
                saver.save(sess, model_path, global_step=step)
                acc_rate += 0.01
                if acc_rate > 0.5: break
        step += 1
    sess.close()
